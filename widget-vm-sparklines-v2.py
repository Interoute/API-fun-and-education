...
#   Name: widget-vm-sparklines-v2.py
#   Purpose: GUI widget to display VM state with sparkine graphs for CPU loading
#   Requires: class VDCApiCall in the file vdc_api_call.py
#   Requires: Sparkplot.py (which requires Matplotlib)
#
#   Requires: Python Imaging Library (may require to be installed)
#
...
# (2) Put this file and the files vdc_api_call.py and Sparkplot.py in any location
# (3) You can run this file using the command 'python widget-vm-sparklines-v2.py'
...
class Application(Frame):
    def vmStates_update(self):
        ...
        vmcounter=0
        self.vmStatesText.imageRefs = [] # used to store references to image objects
        for vm in result['virtualmachine']:
            vmcounter = vmcounter + 1
            if vm['state'] == 'Running':
               data_per_vm = [d[1] for d in data if d[0]==vm['name']]
               #create sparkplot image in temporary file "data.png"
               Sparkplot.Sparkplot(data=[d[1] for d in data_per_vm],label_min=True,label_max=True,plot_min=True,plot_max=True).plot_sparkline()
               self.vmStatesText.insert('end',"\n  [%2d] %s" % (vmcounter,vm['name']), ('stateRunning'))
               #add the content of "data.png" to the Text widget
               fp = open("data.png", "rb")
               imgsrc = Image.open(fp) # open from file object
               imgsrc.load() # make sure PIL has read the data
               dataimg = ImageTk.PhotoImage(imgsrc)
               self.vmStatesText.imageRefs.append(dataimg) # create a reference so image object is not lost in garbage collection
               self.vmStatesText.image_create('end', image=dataimg)
               fp.close()
...
root = Tk()
root.title("VM CPU Sparklines widget version 2")
